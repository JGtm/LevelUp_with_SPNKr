...
async def _get_tokens_from_env_or_args(args: argparse.Namespace) -> Tokens:
    ...
    azure_redirect_uri = (
        args.azure_redirect_uri or os.environ.get("SPNKR_AZURE_REDIRECT_URI") or "https://localhost"
    )
    ...
    clearance = _normalize_token_value(
        args.clearance_token or os.environ.get("SPNKR_CLEARANCE_TOKEN")
    )
...
def _asset_ref(match_info: dict[str, Any], key: str) -> tuple[str | None, str | None]:
...
def _seed_asset_seen_from_db(
    con: sqlite3.Connection, tables: list[str]
...
def _load_match_json_from_db(con: sqlite3.Connection, match_id: str) -> dict[str, Any] | None:
    try:
        cur = con.cursor()
...
async def _import_assets_for_match_info(
    ...
    *,
    ...
    if (
        map_aid
        and map_vid
        and ("Maps", map_aid, map_vid) not in asset_seen
        and ("Maps", map_aid, map_vid) not in asset_missing
    ):
        ...
        try:
            ...
            if isinstance(obj, dict):
                cur.execute(
                    "INSERT INTO Maps(ResponseBody) VALUES (?)",
    ...
    if (
        pl_aid
        and pl_vid
        and ("Playlists", pl_aid, pl_vid) not in asset_seen
        and ("Playlists", pl_aid, pl_vid) not in asset_missing
    ):
        ...
        try:
            ...
            if isinstance(obj, dict):
                cur.execute(
                    "INSERT INTO Playlists(ResponseBody) VALUES (?)",
                    (json.dumps(obj, ensure_ascii=False),),
                )
    ...
    if (
        mp_aid
        and mp_vid
        and ("PlaylistMapModePairs", mp_aid, mp_vid) not in asset_seen
        and ("PlaylistMapModePairs", mp_aid, mp_vid) not in asset_missing
    ):
    ...
    if (
        gv_aid
        and gv_vid
        and ("GameVariants", gv_aid, gv_vid) not in asset_seen
        and ("GameVariants", gv_aid, gv_vid) not in asset_missing
    ):
        ...
        try:
            ...
            if isinstance(obj, dict):
                cur.execute(
                    "INSERT INTO GameVariants(ResponseBody) VALUES (?)",
                    (json.dumps(obj, ensure_ascii=False),),
                )
...
async def main_async(argv: list[str]) -> int:
    ...
    ap.add_argument(
        "--start", type=int, default=0, help="Offset dans l'historique (0 = plus récent)"
    )
    ap.add_argument(
        "--requests-per-second", type=int, default=5, help="Rate limit SPNKr (par service)"
    )
    ap.add_argument(
        "--resume", action="store_true", help="Reprend sur une DB existante (sans effacer)"
    )

    ap.add_argument(
        "--no-skill", action="store_true", help="N'importe pas PlayerMatchStats (skill)"
    )
    ap.add_argument(
        "--no-assets",
        action="store_true",
        help="N'importe pas les assets UGC (Maps/Playlists/etc.)",
    )
    ...
    try:
        ...
        if delta_mode:
            print("[DELTA] Mode delta activé. Arrêt au premier match connu.")
            if not existing_match_ids:
                print(
                    "[DELTA] ⚠️  ATTENTION: aucun match existant, le delta ne pourra pas s'arrêter!"
                )
