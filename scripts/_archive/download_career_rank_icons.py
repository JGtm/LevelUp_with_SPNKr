    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        for rank_data in ranks:
            ...
            if out_path.exists() and not force:
                ...
                tasks.append(
                    (rank_num, download_icon(session, rank_num, icon_large, out_path, headers))
                )
        ...
        for i in range(0, total, 10):
            ...
            for (rank_num, _), result in zip(batch, results, strict=False):
                if result == "ok":
                    downloaded += 1
                elif result == "skip":
                    skipped += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Telecharge les icones de Career Rank Halo Infinite"
    )
    parser.add_argument(
        "--force", "-f", action="store_true", help="Re-telecharger meme si le fichier existe"
    )
    parser.add_argument(
        "--metadata-only", "-m", action="store_true", help="Ne telecharger que les metadonnees"
    )
