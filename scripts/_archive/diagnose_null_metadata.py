        ...
        try:
            ...
            if map_id:
                map_result = ref_conn.execute(
                    "SELECT public_name FROM maps WHERE asset_id = ? LIMIT 1", [map_id]
                ).fetchone()
                if map_result:
            ...
            if playlist_id:
                playlist_result = ref_conn.execute(
                    "SELECT public_name FROM playlists WHERE asset_id = ? LIMIT 1", [playlist_id]
                ).fetchone()
                if playlist_result:
    ...
    try:
        ...
        for row in result:
            ...
            if metadata_db_path:
                ...
                if any(resolved.values()):
                    print("  📋 Résolution depuis refdata:")
                    ...
                    print("  ❌ Aucune résolution possible depuis refdata")
        ...
        for row in recent:
            ...
            if nulls:
                ...
                print(
                    f"  IDs présents: map_id={row_dict['map_id'] is not None}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metadata-db", help="Chemin optionnel vers metadata.duckdb pour vérifier la résolution"
    )
