...
from src.config import DEFAULT_PLAYER_XUID, get_default_db_path
from src.db.connection import get_connection
from src.db.parsers import coerce_duration_seconds, parse_iso_utc
from src.db.parsers import coerce_number as _coerce_number_base
...
def compute_match_performance_score(
    ...
    score = sum(c * w for c, w in zip(components, weights, strict=False)) / total_weight
...
def migrate_match_cache(
    ...
    for table in ["Maps", "Playlists", "PlaylistMapModePairs", "GameVariants"]:
        ...
        try:
            cur.execute(
                f"SELECT json_extract(ResponseBody, '$.AssetId'), json_extract(ResponseBody, '$.PublicName') FROM {table}"
            )
...
def resolve_gamertag_to_xuid(
    ...
    cur = con.cursor()
    cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='XuidAliases'")
    if cur.fetchone():
        cur.execute(
...
def update_is_with_friends(
    ...
    cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='MatchCache'")
    if not cur.fetchone():
    ...
    if "known_teammates_count" not in columns:
        ...
        if not dry_run:
            cur.execute(
                "ALTER TABLE MatchCache ADD COLUMN known_teammates_count INTEGER NOT NULL DEFAULT 0"
            )
    ...
    if "is_with_friends" not in columns:
        ...
        if not dry_run:
            cur.execute(
                "ALTER TABLE MatchCache ADD COLUMN is_with_friends INTEGER NOT NULL DEFAULT 0"
            )
